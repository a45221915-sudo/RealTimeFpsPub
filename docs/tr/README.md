...
* İndirilen zip dosyasına çift tıklayarak açın, bağlam menüsünü açmak için `platform-tools`
  klasörüne sağ tıklayın ve ardından _Servisler_ > _Klasörde Yeni Terminal_ seçeneğine tıklayın.
...
### 7. Real-time FPS Monitor'e PROJECT_MEDIA izninin fiilen verilmesi
...
>
```adb -s [6. adımda gösterilen cihaz kimliği] shell appops set com.tribalfs.realtimefps PROJECT_MEDIA allow```
